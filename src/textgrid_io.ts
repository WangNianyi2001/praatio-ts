...
import {
	TextGrid, IntervalTier, PointTier, copyTier, Tier,
} from './textgrid';
...
function parseNormalTextgrid(data: string): TextGrid {
	// Toss header
...
function parseShortTextgrid(data: string): TextGrid {
	const indexList = [];
 ...
 * @ignore
 */
function fillInBlanks(tier: Tier, blankLabel: string = '') {
...
function tierToText(tier: Tier): string {
	let text = '';
	text += `"${tier.name}"\n`;
 ...
 * @param {TextGrid} tg
 */
function prepTgForSaving(tg: TextGrid) {
